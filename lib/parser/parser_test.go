...
func TestParseFile_NQuadsAndNTriples(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			quads, errChan := ParseFile(tc.filePath, ParserOptions{})
...
func TestParseFileUnsupportedExtension(t *testing.T) {
	_, errChan := ParseFile("test_data/test.rdfed", ParserOptions{})
	err := <-errChan
...
func TestParseFileNotFound(t *testing.T) {
	_, errChan := ParseFile("nonexistent.nq", ParserOptions{})
	err := <-errChan
...
func TestParseUnsupportedFormat(t *testing.T) {
	_, errChan := Parse(strings.NewReader(""), ParserOptions{Format: "application/xml"})
	err := <-errChan
	if err == nil || !strings.Contains(err.Error(), "unsupported") {
...
func TestParseWithEmptyFormat(t *testing.T) {
	...
	quads, errChan := Parse(strings.NewReader(data), ParserOptions{})
	var got []interfaces.IQuad
	for q := range quads {
		got = append(got, q)
	}
...
func TestParseWithNQuadsFormat(t *testing.T) {
	data := `<s> <p> <o> .`
	quads, errChan := Parse(strings.NewReader(data), ParserOptions{Format: "application/n-quads"})
	var got []interfaces.IQuad
	for q := range quads {
		got = append(got, q)
	}
...
func TestParseWithTurtleFormat(t *testing.T) {
	...
	quads, errChan := Parse(strings.NewReader(data), ParserOptions{Format: "text/turtle"})
	var got []interfaces.IQuad
	for q := range quads {
		got = append(got, q)
	}
