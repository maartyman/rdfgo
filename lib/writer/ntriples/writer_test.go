...
func TestWriteNTriples(t *testing.T) {
	tests := []struct {
		...
		{
			name: "quad with named graph (ignored)",
			quads: []interfaces.IQuad{
		...
		{
			name: "named and default graph mixed (ignored)",
			quads: []interfaces.IQuad{
		...
		{
			name: "blank node edge cases",
			quads: []interfaces.IQuad{
				mkQuad("http://ex/s", "http://ex/p", datamodel.NewBlankNode("123"), nil),
				mkQuad("http://ex/s", "http://ex/p", datamodel.NewBlankNode("node_with_special_chars!@#"), nil),
			},
			wantLines: []string{
				`<http://ex/s> <http://ex/p> _:123 .`,
				`<http://ex/s> <http://ex/p> _:node_with_special_chars!@# .`,
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := WriteNTriples(dataStream.ToIStream(), nil, Options{})
			if err != nil {
				if !tt.expectErr {
...
func TestWriteNTriples_Errors(t *testing.T) {
	t.Run("writer returns error immediately", func(t *testing.T) {
		...
		_, err := WriteNTriples(dataStream.ToIStream(), writer, Options{})
	...
	t.Run("closed stream behaves cleanly", func(t *testing.T) {
		...
		_, err := WriteNTriples(dataStream.ToIStream(), &buf, Options{})
...
func TestWriteMultipleStores(t *testing.T) {
	...
	for _, store := range stores {
		...
		_, err := WriteNTriples(dataStream, &buf, Options{})
		if err != nil {
	...
	// Additional test for large data sets
	var largeQuads []interfaces.IQuad
	for i := 0; i < 10000; i++ {
		largeQuads = append(largeQuads, mkQuad(
			fmt.Sprintf("http://ex/s%d", i),
			"http://ex/p",
			datamodel.NewNamedNode(fmt.Sprintf("http://ex/o%d", i)),
			nil,
		))
	}

	largeStream := stream.ArrayToStream(largeQuads)
	var largeBuf bytes.Buffer
	_, err := WriteNTriples(largeStream.ToIStream(), &largeBuf, Options{})
	if err != nil {
		t.Fatalf("unexpected error writing large data set: %v", err)
	}

	if largeBuf.Len() == 0 {
		t.Error("expected output for large data set but got empty buffer")
	}
}
